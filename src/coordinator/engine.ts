...
export interface CoordinatorConfig {
  ...
  model?: string;
  verbose?: boolean;
  // Override the worker spawner — receives instruction + returns result text
  // If not provided, uses a default LLM completion call
  workerRunner?: (task: WorkerTask, ctx: { scratchDir: string; system?: string }) => Promise<string>;
}
...
async function defaultWorkerRunner(
  task: WorkerTask,
  ctx: { scratchDir: string; system?: string },
): Promise<string> {
...
When done, write your findings to ${ctx.scratchDir}/worker-${task.id}.md`;
  ...
  const response = await (ctx as unknown as { llm: LLMClient }).llm.complete({
...
export class CoordinatorEngine {
  ...
  private async runPhase(
    ...
    try {
      ...
      let workerOutputs: Record<string, string> | undefined;
      if (workerTasks && workerTasks.length > 0) {
        ...
        const workerPromises = workerTasks.map(async (task) => {
          ...
          writeFileSync(
            join(this.scratchDir, `worker-${task.id}-${phase}.md`),
            result,
            'utf-8',
          );
  ...
  async run(
    goal: string,
    workspace: string,
    onPhase?: (result: CoordinatorResult) => void,
  ): Promise<CoordinatorRunResult> {
    this.log(`Starting: ${goal}`);
    const phases: CoordinatorResult[] = [];
    ...
    // Phase 2: Synthesis
    {
      ...
      try {
        ...
        if (existsSync(specPath)) {
          (phases[phases.length - 1] as CoordinatorResult & { spec?: string }).spec =
   ...
   */
  private deriveImplTasks(spec: string): WorkerTask[] {
    if (!spec) return [];
  ...
  private deriveVerifyTasks(_goal: string): WorkerTask[] {
