...
export const COORDINATOR_PHASE_PROMPTS = {
  research: (goal: string, workspace: string, workerCount: number, scratchDir: string) =>
...
- After workers return: a synthesis of their combined findings`,
  ...
  synthesis: (goal: string, workspace: string, scratchDir: string) =>
...
- Full spec written to coordinator-spec.md`,
  ...
  implementation: (goal: string, workspace: string, workerCount: number, scratchDir: string) =>
`## Phase 3 — Implementation (COORDINATOR)
...
- List of ${workerCount} implementation tasks with exact change instructions`,
  ...
  verification: (goal: string, workspace: string, workerCount: number, scratchDir: string) =>
