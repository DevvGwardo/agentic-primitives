...
import { createLockfile, type Lockfile } from './lockfile.js';
...
export interface DreamConfig {
  memoryDir: string;           // e.g. './memory' — where MEMORY.md and logs live
  sessionThreshold?: number;    // min sessions before dreaming (default: 5)
  timeThresholdHours?: number; // min hours since last dream (default: 24)
  maxMemLines?: number;        // max lines in MEMORY.md (default: 200)
  maxMemKb?: number;           // max KB in MEMORY.md (default: 25)
  lockPath?: string;           // default: memoryDir/.dream.lock
  llm: LLMClient;              // LLM client (Anthropic, OpenAI, etc.)
  dryRun?: boolean;            // run phases but don't write files (default: false)
  verbose?: boolean;
}

export interface DreamResult {
  ran: boolean;
  reason?: string;          // why it ran or didn't
  phasesRun?: string[];     // ['orient', 'gather', 'consolidate', 'prune']
  signalsFound?: number;
...
export class DreamEngine {
  ...
  private state: DreamStateTracker;
   ...
   */
  checkGates(): { pass: boolean; reason: string } {
   ...
   */
  private async readMemoryStructure(): Promise<string> {
    const entries = readdirSync(this.config.memoryDir, { withFileTypes: true });
    const lines: string[] = ['## Memory Directory Structure\n'];

    for (const entry of entries.sort((a, b) => a.name.localeCompare(b.name))) {
      if (entry.name.startsWith('.')) continue; // skip hidden files like .dream.lock
   ...
   *                         ```
   */
  private parseFileWriteDirective(content: string): Array<{ path: string; body: string }> {
   ...
   */
  async maybeDream(): Promise<DreamResult> {
    // Gate check
   ...
   */
  async runForced(): Promise<DreamResult> {
    if (!this.tryAcquireLock()) {
      return { ran: false, reason: 'Lock gate closed: another dream is running' };
    }
    try {
  ...
  private async run(): Promise<DreamResult> {
    this.log('Dream starting');
    const phasesRun: string[] = [];
    const phaseContexts: string[] = [];
    const phaseContents: PhaseResult[] = [];
    ...
    // Phase 1: Orient
    {
      const result = await this.runPhase(
        'orient',
        DREAM_PHASE_PROMPTS.orient(this.config.memoryDir),
        memoryStructure,
      );
    ...
    // Phase 2: Gather Signal
    {
      ...
      const result = await this.runPhase(
        'gather',
        DREAM_PHASE_PROMPTS.gatherSignal(this.config.memoryDir),
        gatherContext,
      );
      phasesRun.push(result.phase);
      phaseContexts.push(result.content);
    ...
    const consolidateResult = await this.runPhase(
      'consolidate',
      DREAM_PHASE_PROMPTS.consolidate(this.config.memoryDir),
      consolidateContext,
    );
    ...
    const pruneResult = await this.runPhase(
      'prune',
      DREAM_PHASE_PROMPTS.prune(this.config.memoryDir),
      pruneContext,
    );
